...
function syncStages(originalCode, pluginOptions, options, prepareStageResult, babelConfig, cache, eventEmitter = EventEmitter.dummy) {
  const {
    filename
  } = options;
  ...
  if (ast === 'ignored' || !prepareStageResult || !withLinariaMetadata(prepareStageResult.metadata)) {
    return {
      code: originalCode,
  ...
  const finishStage2Event = eventEmitter.pair({
    stage: 'stage-2',
    filename
  });
  const evalStageResult = evalStage(cache, prepareStageResult.code, pluginOptions, filename);
  finishStage2Event();
  ...
  const finishStage3Event = eventEmitter.pair({
    stage: 'stage-3',
    filename
  });
  const collectStageResult = prepareForRuntime(babel, ast, originalCode, valueCache, pluginOptions, options, babelConfig);
  finishStage3Event();
  ...
  const finishStage4Event = eventEmitter.pair({
    stage: 'stage-4',
    filename
  });
...
export function transformSync(originalCode, options, syncResolve, babelConfig = {}, cache = new TransformCacheCollection(), eventEmitter = EventEmitter.dummy) {
  const {
    filename
  } = options;
  // *** 1st stage ***
  ...
  const prepareStageResults = prepareForEvalSync(babel, cache, syncResolve, entrypoint, pluginOptions, options);
  finishEvent();

  // *** The rest of the stages are synchronous ***

  return syncStages(originalCode, pluginOptions, options, prepareStageResults, babelConfig, cache, eventEmitter);
}
export default async function transform(originalCode, options, asyncResolve, babelConfig = {}, cache = new TransformCacheCollection(), eventEmitter = EventEmitter.dummy) {
  const {
    filename
  } = options;
  ...
  const pluginOptions = loadLinariaOptions(options.pluginOptions);
  const prepareStageResults = await prepareForEval(babel, cache, asyncResolve, entrypoint, pluginOptions, options, eventEmitter);
  finishEvent();
